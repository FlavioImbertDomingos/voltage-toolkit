...
from descriptions import PANELS, ROWS  # noqa: E402

OUT = Path(__file__).parent / "dashboards" / "voltage.json"
DOC = Path(__file__).parent.parent / "docs" / "DASHBOARD.md"
...
def describe(panel: dict) -> None:
    """Attach the (i) tooltip from descriptions.py. Fails loudly for a panel nobody explained."""
    title = panel["title"]
    if panel["type"] == "row":
        if title not in ROWS:
            raise SystemExit(f"grafana/descriptions.py: no ROWS entry for row {title!r}")
        return
    if title not in PANELS:
        raise SystemExit(f"grafana/descriptions.py: no PANELS entry for panel {title!r}")
    what, why, source = PANELS[title]
    panel["description"] = f"{what}\n\nWhy it matters: {why}\n\nSource: {source}"


def render_doc() -> str:
    """docs/DASHBOARD.md, generated so it cannot drift from the panels."""
    out = [
    ...
    for panel in panels:
        title = panel["title"]
        if panel["type"] == "row":
            out += [f"## {title}", "", ROWS[title], ""]
            continue
        what, why, source = PANELS[title]
        kind = {"stat": "tile", "timeseries": "graph", "table": "table"}.get(
...
print(f"wrote {OUT} ({len(panels)} panels) and {DOC}")
